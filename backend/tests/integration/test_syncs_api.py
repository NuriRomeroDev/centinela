...
async def _seed_sync_with_files(
    db_session, *, estado=SyncEstado.completed, rejected=0, total=3, **kw
):
...
async def test_syncs_include_files_is_query_count_constant(api_client, db_session, query_counter):
    for i in range(60):
        await _seed_sync_with_files(
            db_session, estado=SyncEstado.failed if i % 3 else SyncEstado.completed, rejected=i % 3
        )
...
async def test_remediations_list_newest_first_with_correlation_id(
    ...
    assert len(rows) == 40
    assert [row["accion_ejecutada"] for row in rows] == [
        f"ACC_{i:02d}" for i in range(40)
    ]  # newest first
...
async def test_remediate_retry_job_transitions_failed_to_running(api_client, db_session):
    ...
    response = await api_client.post(
        f"{SYNCS_URL}/{sync.id}/remediate", json={"accion": "RETRY_JOB"}
    )
    assert response.status_code == 200
...
async def test_remediate_unsupported_accion_returns_422(api_client, db_session):
    ...
    response = await api_client.post(
        f"{SYNCS_URL}/{sync.id}/remediate", json={"accion": "PURGE_ALL"}
    )
    assert response.status_code == 422
    assert response.json()["error"]["codigo"] == "ERR_VALIDATION"
